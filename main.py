...
if __name__ == "__main__":
    ...
    if generate_and_save:
        ...
        groups = cluster_images(np.concatenate([X_unlabel, X_val], axis=0), encoder, plot=True)
        X_unlabel_groups = groups[:len(X_unlabel)]
        X_val_groups = groups[len(X_unlabel):]
        
        f = open('/opt/workspace/host_storage_hdd/mnist_preprocessed.pickle', 'wb')
        pickle.dump({
            'X_train': X_train,
            'y_train': y_train,
            'X_unlabel': X_unlabel,
        ...
        data = pickle.load(open('/opt/workspace/host_storage_hdd/mnist_preprocessed.pickle', 'rb'))
