...
class VRAE(BaseEstimator, nn.Module):
    ...
    def compute_loss(self, X):
        ...
        """
        x = torch.tensor(X[:,:,:].type(self.dtype), requires_grad = True)
    ...
    def fit(self, dataset, validation_set=None, save = False):
        """
        Calls `_train` function over a fixed number of epochs, specified by `n_epochs`
        :param dataset: `Dataset` object containing training samples
        :param validation_set: `Dataset` object containing validation samples
            ...
            if validation_set is not None:
                ...
                with torch.no_grad():
                    for [val_batch] in val_loader:
                        ...
                        val_losses.append(self.compute_loss(val_batch)[:-1])
                self.train()
                
                val_losses = torch.tensor(val_losses).mean(dim=0)
                print("Validation losses, loss = %.4f, recon_loss = %.4f, kl_loss = %.4f" % (val_losses[0], val_losses[1], val_losses[2]))
    ...
    def _batch_transform(self, x):
        ...
        """
        return self.lmbd(self.encoder(torch.tensor(x.type(self.dtype), requires_grad = False))).cpu().data.numpy()
    ...
    def _batch_reconstruct(self, x):
        ...
        x = torch.tensor(x.type(self.dtype), requires_grad = False)
        x_decoded, _ = self(x)
...
def get_episodes(samples, y, plot=False):
    dones = [d for s, a, r, _s, d in samples]
    if plot:
        plt.plot(np.arange(len(dones)), dones)
        plt.savefig("tmp.svg")

    dones = np.where(dones)[0]
    
    episodes = None
    ys = []
    for i in range(len(dones)-1):
        episode_s = []
        for j in range(dones[i]+1, dones[i+1]+1):
            episode_s.append(np.concatenate([samples[j][0], samples[j][1], [samples[j][2]]]))
        
        if episodes is None:
            episodes = np.array([episode_s])
        else:
            episodes = np.append(episodes, np.array([episode_s]), axis=0)
            
        ys.append(y[i])
        
    return np.array(episodes), np.array(ys)


def stretch_episodes(whole_episodes, y, history_size=50):
    stretched_episodes = []
    ys = []
    for episode, _y in zip(whole_episodes, y):
        stretched_episode = np.zeros((history_size, episode.shape[1]))
        for i in range(-history_size + 1, len(episode)-history_size):
            if i < 0:
                stretched_episode[-(history_size + i):] = episode[0:(history_size + i)]
                stretched_episodes.append(stretched_episode)
            else:
                stretched_episodes.append(episode[i:i+history_size])
            ys.append(_y)
            
    stretched_episodes = np.array(stretched_episodes)
    ys = np.array(ys)
    return stretched_episodes, ys


def pad_states(samples, n_classes=10):
    for i, sample in enumerate(samples):
        s = np.zeros((n_classes, n_classes))
        _s = np.zeros((n_classes, n_classes))
        
        old_s = sample[0][:-5]
        old_s = old_s.reshape((int(np.sqrt(len(old_s))), int(np.sqrt(len(old_s)))))
        s[:old_s.shape[0], :old_s.shape[1]] = old_s
        s = s.reshape(-1)
        s = np.concatenate([s, sample[0][-5:]])
        
        _old_s = sample[3][:-5]
        _old_s = _old_s.reshape((int(np.sqrt(len(_old_s))), int(np.sqrt(len(_old_s)))))
        _s[:_old_s.shape[0], :_old_s.shape[1]] = _old_s
        _s = _s.reshape(-1)
        _s = np.concatenate([_s, sample[3][-5:]])
        
        samples[i] = (s, sample[1], sample[2], _s, sample[4])
        
    return samples


def episodes_split(episodes, dataset_y, trainsplit, valsplit, testsplit):
    trainsplit = int(trainsplit * len(episodes))
    valsplit = int(valsplit * len(episodes))
    
    X_train, X_val, X_test = None, None, None
    y_train, y_val, y_test = None, None, None
    for dataset_i in np.unique(dataset_y):
        mask = dataset_i == dataset_y
        dataset_episodes = episodes[mask]
        dataset_ys = dataset_y[mask]
        if X_train is None:
            X_train = dataset_episodes[:trainsplit]
            X_val = dataset_episodes[trainsplit:trainsplit + valsplit]
            X_test = dataset_episodes[trainsplit + valsplit:]
            y_train = dataset_ys[:trainsplit]
            y_val = dataset_ys[trainsplit:trainsplit + valsplit]
            y_test = dataset_ys[trainsplit + valsplit:]
        else:
            X_train = torch.cat(X_train, dataset_episodes[:trainsplit], axis=0)
            X_val = torch.cat(X_val, dataset_episodes[trainsplit:trainsplit + valsplit], axis=0)
            X_test = torch.cat(X_test, dataset_episodes[trainsplit + valsplit:], axis=0)
            y_train = torch.cat(y_train, dataset_ys[:trainsplit], axis=0)
            y_val = torch.cat(y_val, dataset_ys[trainsplit:trainsplit + valsplit], axis=0)
            y_test = torch.cat(y_test, dataset_ys[trainsplit + valsplit:], axis=0)
...
if __name__ == '__main__':
    ...
    dload = '/opt/workspace/host_storage_hdd/lstm_autoencoder'
    buffer_size = 250000
    history_size = 50
    
    buffer_loads = [
        ('/opt/workspace/host_storage_hdd/results/2020-03-15_13-34-58_ag_news/', 17500),
        ('/opt/workspace/host_storage_hdd/results/2020-03-17_17-19-27_cifar10/', 17500)
    ]
    
    samples = []
    dataset_y = []
    for i, (buffer_path, last_n_samples) in enumerate(buffer_loads):
        buffer_path = '/opt/workspace/host_storage_hdd/results/2020-03-15_13-34-58_ag_news/'
        replay_buffer = ReplayBuffer(buffer_size)
        replay_buffer.load_buffer(buffer_path)
        _samples = replay_buffer.buffer[-last_n_samples:]
        _samples = pad_states(_samples, n_classes=10)
        
        dataset_y += [i] * len(_samples)
        samples += _samples
    
    episodes, dataset_y = get_episodes(samples, dataset_y, plot=True)
    episodes, dataset_y = stretch_episodes(episodes, dataset_y, history_size=history_size)
    episodes = torch.tensor(episodes)
    dataset_y = torch.tensor(dataset_y)

    X_train, y_train, X_val, y_val, X_test, y_test = episodes_split(episodes, dataset_y, 0.7, 0.1, 0.2)
    X_train, X_val, X_test = TensorDataset(X_train), TensorDataset(X_val), TensorDataset(X_test)
    
    vrae = VRAE(sequence_length=episodes.shape[1],
                ...
                loss=loss,
                dload=dload)
    vrae.fit(X_train, X_val)
    
    z = vrae.transform(X_val)
    kmeans = KMeans(n_clusters=len(np.unique(dataset_y)))
